...
var sumOfProperDivisorsExamples = []struct {
	input    uint
	expected uint
}{
	{1, 0},
...
func TestSumOfProperDivisors(t *testing.T) {
	for _, tc := range sumOfProperDivisorsExamples {
...
func TestIsPerfectNumber(t *testing.T) {
	for _, tc := range perfectNumberExamples {
		if !IsPerfectNumber(tc.perfectNumber) {
			t.Errorf("%d is expected to be a perfect number.", tc.perfectNumber)
		}
	}
}

var notPerfectNumberExamples = []struct {
	notPerfectNumber uint
}{
	{0},
	{1},
	{2},
...
func TestIsNotPerfectNumber(t *testing.T) {
	for _, tc := range notPerfectNumberExamples {
		if IsPerfectNumber(tc.notPerfectNumber) {
			t.Errorf("%d is expected not to be a perfect number.", tc.notPerfectNumber)
		}
	}
}
